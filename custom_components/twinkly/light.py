...
EP_TIMEOUT = ClientTimeout(
    total=3
)  # It on LAN, and if too long we will get warning to the update duration in logs
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    async_add_entities(
        [TwinklyLight(config.get(CONF_NAME), config[CONF_HOST], session)], True
    )
...
class TwinklyLight(LightEntity):
    ...
    def name(self) -> str:
        ...
        return (
            self._name
            if self._name
            else self._attributes[ATTR_NAME]
    ...
    async def async_update(self) -> None:
        ...
        try:
            ...
            self._brightness = (
                (await self.get_brigthness()) * 2.55 if self._is_on else 0
            )
